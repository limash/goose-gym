...
def get_feature_maps(config, state, old_heads):
    ...
    for idx, goose in enumerate(state['geese']):
        ...
        A[0 + idx * n_geese, goose[:1]] = 1  # head
        A[1 + idx * n_geese, :] = old_heads[idx, :]  # previous head
        old_heads[idx, :] = A[0 + idx * n_geese, :]
        # A[n_geese + idx, goose[-1:]] = 1  # tail
        A[2 + idx * n_geese, goose[-1:]] = 1  # tail
        # A[2 * n_geese + idx, goose] = 1  # body
        A[3 + idx * n_geese, goose] = 1  # body
