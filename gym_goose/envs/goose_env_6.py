...
class GooseEnv(gym.Env, ABC):
    ...
    def __init__(self, debug=False):
        ...
        observation = spaces.Box(low=0, high=1,
                                 shape=(self._n_agents, (self._config.rows + self._config.columns) * 4),
                                 dtype=np.uint8)
        scalars = spaces.Box(low=0,
                             high=1,
                             shape=(self._binary_positions + self._config.rows + self._config.columns,),
                             dtype=np.uint8)
    ...
    def get_players_obs(self, state, players_obs):
        ...
        for coord in state[0].observation['food']:
            row, col = row_col(coord, self._env.configuration.columns)
            food[row] = 1
            food[self._env.configuration.rows + col] = 1
        for i in range(self._n_agents):
            ...
            time_bin = to_binary(time_step, self._binary_positions).ravel()
            scalars = np.concatenate([food, time_bin])
            # store observations
...
def get_feature_maps(config, state, old_heads):
    n_geese = len(state['geese'])
    # head, tail, body, previous head
    number_of_layers = n_geese
    A = np.zeros((number_of_layers, 4, config.rows + config.columns), dtype=np.uint8)
    for idx, goose in enumerate(state['geese']):
        try:
            heads = row_col(goose[0], config.columns)
            A[idx, 0, heads[0]] = 1  # heads row
            A[idx, 0, config.rows + heads[1]] = 1  # heads col
            tails = row_col(goose[-1], config.columns)
            A[idx, 1, tails[0]] = 1  # tail row
            A[idx, 1, config.rows + tails[1]] = 1  # tail col
            for coord in goose:
                row, col = row_col(coord, config.columns)
                A[idx, 2, row] = 1  # body row
                A[idx, 2, config.rows + col] = 1  # body col
            A[idx, 3, :] = old_heads[idx, :]
        except IndexError:
            pass

    B = A.reshape((number_of_layers, -1))
